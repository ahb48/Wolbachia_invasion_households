function  [props] = setup_wolbachia(props)
...
% Ben Adams, 2024, based on original code by
% Merten Stender
...
props.progessBar = false; % tool does not work anymore due to upd removal
...
props.model.odeFun = @ode_wolbachia; % a function handle @my_ode_function
...
props.model.odeParams = [b, d, d_prime, phi, h, k, C, u, v];
...
props.roi.N = 1000; % integer number
...
props.roi.maxLimits = [30, 30]; % must be of length <props.model.dof>
...
props.clust.featExtractFun = @features_wolbachia; % a function handle
...
props.templ.Y0{1} = [8; 0];   % initial condition (NOT the steady-state itself)
props.templ.modelParams{1} = [b, d, d_prime, phi, h, k, C, u, v]; % model parameters
props.templ.label{1} = 'M0';    % stable fixed point label

% the initial condition that leads the system to end up on the second stable steady state
props.templ.Y0{2} = [0; 8];
props.templ.modelParams{2} = [b, d, d_prime, phi, h, k, C, u, v];
props.templ.label{2} = '0W'; % limit cycle solution label
